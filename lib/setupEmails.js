...
module.exports = function (app, config, dbWith) {
	...
	var doHolibirhtdaysForDate = function (date, holibirtherEvent, friendsEvent) {
		...
		return db.holibirthday.find({
			...
			return db.siteCopyItem.findOne({
				uniqueName: 'Automated Emails Unsubscribe Footer (wrap text for "unsubscribe" link in double curly braces, e.g. {{unsubscribe}} )',
			}, function (copyItem) {
				return db.profile.find({
									...
									return db.mailchimpTemplate.findOneOrFail({
										...
										mc.templates.info({
											...
											mc.templates.info({
												...
												var addLink = function (str, profile) {
													var openPos = str.indexOf('{{');
													var closePos = str.indexOf('}}');
													return str.substring(0, openPos) +
														'<a href="' +
														config.domain +
														'/#!optOutEmails?token=' +
														profile.optOutToken +
														'">' +
														str.substring(openPos + 2, closePos) +
														'</a>' +
														str.substring(closePos + 2);
												};
												var ensureHasToken = function (f) {
													return function (p) {
														if (p.optOutToken) {
															return f(p);
														}
														else {
															p.optOutToken = generateToken();
															db.profile.update({
																_id: p._id,
															}, {
																$set: {
																	optOutToken: p.optOutToken,
																}
															});
															return f(p);
														}
												...
												var emails = holibirtherProfiles.filter(function (p) {
												...
												}).map(ensureHasToken(function (p) {
													var source = holibirtherTemplate.source + '<br>' + addLink(copyItem.value, p);
													console.log(holibirtherTemplate.source);
													return {
														to: p.email,
													...
													var source = friendsTemplate.source + '<br>' + addLink(copyItem.value, p);
													console.log(friendsTemplate.source);
													return {
														to: p.email,
	...
	schedule.scheduleJob('16 0 0 * * *', function () {
		...
		var today = new Date();
		doHolibirhtdaysForDate(today.getDate(),
							   app.schema.mailchimpTemplate.fields.event.options.holibirthdayToday,
							   app.schema.mailchimpTemplate.fields.event.options.friendsHolibirthdayToday);
	});
};
